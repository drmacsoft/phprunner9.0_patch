<?php
$OutsourceFile = dirname(dirname(__FILE__)) . '/outsource/outsource_init.php';
if (file_exists($OutsourceFile)) {
    include $OutsourceFile;
}
/**
...
##if @BUILDER.nLoginMethod==SECURITY_TABLE && @BUILDER.bCreateLoginPage##
...
{
	$langArr = array();
##foreach @BUILDER.arrLanguages as @l##
	$langArr["##@l##"] = "##langfile @l##";
##endfor##
...
##foreach @BUILDER.Tables as @t##
	if ("##@t.strShortTableName##" == $shortTName && ($type===false || ($type!==false && $type == ##@t.nType##)))
		return true;
##endfor##
	return false;
}
...
{
##foreach @BUILDER.tables as @t filter @t.nType==titCHART##
	if($shorttable=="##@t.strShortTableName##")
		return "##@t.strChartType c##";
##endfor##
	return "";
}
...
function ReadUserPermissions($userID = "")
{
	...
	if($userID != "Guest")
	{
##if @BUILDER.nLoginMethod==LOGIN_AD##
		...
		foreach($userGroups as $g)
		{
			...
			if($caseInsensitiveUsername)
				$groupstr.= strtoupper( $grConnection->prepareString($g) );
			else
				$groupstr.= $grConnection->prepareString($g);
			if($g == -1)
				$bIsAdmin = true;
		}
		...
		if($caseInsensitiveUsername)
			...
			." where ".$grConnection->upper( $grConnection->addFieldWrappers( "##@BUILDER.groups_label s##" ) )." in (".$groupstr.")";
		else
			$sql = "select ". $grConnection->addFieldWrappers( "##@BUILDER.groups_groupid s##" )
			." from ". $grConnection->addTableWrappers( "##@BUILDER.strUGGroups s##" )
			." where ".$grConnection->addFieldWrappers( "##@BUILDER.groups_label s##" )." in (".$groupstr.")";
##else##
		$grConnection = $cman->getForUserGroups();

		if(!$caseInsensitiveUsername)
		{
			$sql = "select ".$grConnection->addFieldWrappers( "##@BUILDER.members_groupid s##" )
				.", ".$grConnection->addFieldWrappers( "##@BUILDER.members_username s##" )
				." from ". $grConnection->addTableWrappers( "##@BUILDER.strUGMembers s##" )
				." where ".$grConnection->addFieldWrappers( "##@BUILDER.members_username s##" )."=".$grConnection->prepareString($userID);
		}
		else
		{
			$sql = "select ".$grConnection->addFieldWrappers( "##@BUILDER.members_groupid s##" )
				.", ".$grConnection->addFieldWrappers( "##@BUILDER.members_username s##" )
				." from ". $grConnection->addTableWrappers( "##@BUILDER.strUGMembers s##" )
				." where ".$grConnection->addFieldWrappers( "##@BUILDER.members_username s##" )."=".strtoupper( $grConnection->prepareString($userID) );
		}
##endif##
...
##endif##
	...
	$sql = "select ". $rConnection->addFieldWrappers( "##@BUILDER.rights_tablename s##" )
		.", ". $rConnection->addFieldWrappers( "##@BUILDER.rights_accessmask s##" )
		." from ". $rConnection->addTableWrappers( "##@BUILDER.strUGRights s##" )
		." where ". $rConnection->addFieldWrappers( "##@BUILDER.rights_groupid s##" ) ." in (".$groupstr.")";
...
function guestHasPermissions()
{
	...
	if(!count($_SESSION["UserRights"]["Guest"]))
		return false;
##foreach @BUILDER.tables as @t##
	if(array_key_exists("##@t.strDataSourceTable s##",$_SESSION["UserRights"]["Guest"]))
		return true;
##endfor##
	return false;
}
...
##else## //the bDynamicPermissions block
...
##foreach @BUILDER.UserGroups as @u filter @u.strUserName!="<Default>"##
	if($table=="##@t.strDataSourceTable s##" && $sUserGroup=="##@u.strUserName s##")
	{
	##if @u.strUserName=="<Guest>" && @t.nSecOptions==ADVSECURITY_VIEW_OWN##
		// guest sees nothing
		return "";
	##else##
		return "##@u.TablePermissions[strTableName=@t.strDataSourceTable].strPermissionMask##".$extraPerm;
	##endif##
	}
##endfor##
//	default permissions
...
##if @BUILDER.UserGroups[strUserName=="<Default>"].len##
		return "##@BUILDER.UserGroups[strUserName=="<Default>"].TablePermissions[strTableName=@t.strDataSourceTable].strPermissionMask##".$extraPerm;
...
function IsAdmin()
{
##if @BUILDER.bCreateLoginPage && @BUILDER.bDynamicPermissions##
...
##if @BUILDER.bCreateLoginPage##
	if( !IsLogged() )
		return "";
##endif##
##if @BUILDER.bCreateLoginPage && @BUILDER.nLoginMethod!=LOGIN_HARDCODED##
	##if @BUILDER.bDynamicPermissions##
...
##if FBLoginMethod##
	...
	if($scriptname != GetTableLink("login") && $scriptname != GetTableLink("remind") && $scriptname != GetTableLink("register") )
	{
		if (inFacebook())
			return true;
	}
...
##if @BUILDER.nLoginMethod==LOGIN_AD##
	return false;
##else##
	global $cman, $cCharset, $cUserNameField, $cPasswordField;
	...
	if ($signed_request)
	{
		...
		$strSQL = "select * from ".$connection->addTableWrappers("##@BUILDER.strLoginTable s##")
			." where ".$connection->addFieldWrappers(GetUserNameField())."='".$strUsername."'";
...
{
	global $globalEvents;
##if GroupSecurity && @BUILDER.strUserGroupField##
	$_SESSION["GroupID"] = $data["##@BUILDER.strUserGroupField s##"];
	##foreach @BUILDER.UserGroups as @u filter @u.bAdmin##
	if($_SESSION["GroupID"]=="##@u.strUserName s##")
...
##if @BUILDER.bAdminUser##
	$cAdminUserID = "##@BUILDER.strAdminUserID s##";
	if($pUsername==$cAdminUserID)
		$_SESSION["AccessLevel"] = ACCESS_LEVEL_ADMIN;
##endif##

##foreach @BUILDER.tables as @t filter @t.strUsersTableOwnerID##
	##if @first##
	$_SESSION["OwnerID"] = $data["##@t.strUsersTableOwnerID s##"];
	##endif##
	$_SESSION["_##@t.strDataSourceTable s##_OwnerID"] = $data["##@t.strUsersTableOwnerID s##"];
##endfor##
...
function DoLogin($callAfterLoginEvent = false, $userID = "Guest", $userName = "", $groupID = "<Guest>", $accessLevel = ACCESS_LEVEL_GUEST, $password = "", &$pageObject = null)
{
	...
	if($userID == "Guest" && $userName == "")
		$userName = ##message AA_GROUP_GUEST##;
...
##if @BUILDER.bCreateLoginPage##
	...
	{
##foreach @BUILDER.Tables as @t filter @t.nSecOptions==ADVSECURITY_VIEW_OWN || @t.nSecOptions==ADVSECURITY_EDIT_OWN##
		if($table=="##@t.strDataSourceTable s##")
		{

	##if @t.nSecOptions==ADVSECURITY_VIEW_OWN##
			if(!($pSet->getCaseSensitiveUsername((string)$_SESSION["_".$table."_OwnerID"])===$pSet->getCaseSensitiveUsername((string)$strValue)))
				return false;
	##elseif @t.nSecOptions==ADVSECURITY_EDIT_OWN##
...
##if @BUILDER.bCreateLoginPage && (@BUILDER.nLoginMethod==LOGIN_TABLE || @BUILDER.nLoginMethod==LOGIN_AD)##
...
{
	$len = strlen($pwd);
##if @BUILDER.Registration.nPwdLen##
	if($len < ##@BUILDER.Registration.nPwdLen##)
		return false;
##endif##
	$cUnique = array();
...
##if @BUILDER.Registration.nPwdUnique##
	if(count($cUnique)<##@BUILDER.Registration.nPwdUnique##)
		return false;
##endif##
##if @BUILDER.Registration.nPwdDigits##
	if($cDigit<##@BUILDER.Registration.nPwdDigits##)
		return false;
##endif##
##if @BUILDER.Registration.bPwdUpperLower##
	if(!$cLower || !$cUpper)
		return false;
##endif##
	return true;
...
{
##if @BUILDER.AuditTrailSettings.LoggingMode<=0##
	return NULL;
##endif##
...
##if @BUILDER.AuditTrailSettings.LoggingMode==1##
		return new AuditTrailTable();
##elseif @BUILDER.AuditTrailSettings.LoggingMode==2##
...
{
##if !@BUILDER.AuditTrailSettings.bEnableLocking##
	return NULL;
##endif##
...
##foreach @BUILDER.arrLanguages as @lang##
	$xt->assign("##@lang s##LANGLINK", "##@lang s##" != $currentLang);
##endfor##
...
function getMountNames()
{
	...
	##else##
	$mounts[1] = ##message MONTH_JAN##;
	$mounts[2] = ##message MONTH_FEB##;
	$mounts[3] = ##message MONTH_MAR##;
	$mounts[4] = ##message MONTH_APR##;
	$mounts[5] = ##message MONTH_MAY##;
	$mounts[6] = ##message MONTH_JUN##;
	$mounts[7] = ##message MONTH_JUL##;
	$mounts[8] = ##message MONTH_AUG##;
	$mounts[9] = ##message MONTH_SEP##;
	$mounts[10] = ##message MONTH_OCT##;
	$mounts[11] = ##message MONTH_NOV##;
	$mounts[12] = ##message MONTH_DEC##;
	##endif##
...
{
##if @BUILDER.bDebug && @BUILDER.bBuildMobile##
	return true;
##endif##
##if @BUILDER.bBuildMobile##
...
##if !@BUILDER.bBuildMobile##
	return false;
##else##
	if( isset($_SESSION['mobileDetectionStatus']) )
		$status = $_SESSION['mobileDetectionStatus'];
	else
	{
		$status = MobileDetected();
		// cach the MobileDetected result
		$_SESSION['mobileDetectionStatus'] = $status;
	}

	$isMobile = ( $status >= 1 && $status <= 4 );
##if @BUILDER.bDisplayDesktop##
	if( @$_COOKIE["runnerTabletDevice"] )
		$isMobile = false;
##endif##
	return $isMobile;
##endif##
...
{
##if @BUILDER.bBuildMobile##
	$ovrd = func_Override( $templateFileName );
	return mobileDeviceDetected() && $ovrd != otUseDesktop || $ovrd == otUseMobile;
##else##
	return false;
##endif##
}
...
function GetBaseScriptsForPage($isDisplayLoading, $additionalScripts = "", $customText = "")
{
	$result = "";
##if @ext == "aspx"##
	$result .= "<script type=\"text/javascript\">window.runnerWebRootPath=\"".urlencode(GetWebRootPath())."\"</script>";
##endif##
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/loadfirst.js")."\"></script>";
	$result .= $additionalScripts;
##if @BUILDER.m_bDebug##
	$result .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/runnerJS/RunnerBase.js")."\"></script>";
##endif##
...
##foreach @BUILDER.Tables as @t##
##foreach @t.arrFieldObj as @f filter @f.bFiltered && @f.pFilterFormatObj.strFilterFormat == IntervalistFilterFormat && @f.pFilterFormatObj.arrFilterIntervals.len##
##foreach @f.pFilterFormatObj.arrFilterIntervals as @Interval##
##if @Interval.bLowerUsesExpression##
	if($table=="##@t.strDataSourceTable s##" && $field=="##@f.strName s##" && $isLowerBound && $idx == ##@index##)
	{
		//get the expression for a lower bound
		##@Interval.strLowerLimit##
		;
		return $value;
	}
##endif##
...
{
##if !@BUILDER.bDetectDefaultLanguage##
	return "##@BUILDER.strDefaultLanguage s##";
##else##
