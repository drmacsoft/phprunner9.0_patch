...
class JalaliDate {
    ...
    static function jstrtotime($jalaliStr) {
        $jdatestr = explode(' ', $jalaliStr);
        $jparam = explode('/', $jdatestr);
        return jmaketime(0, 0, 0, $jparam[1], $jparam[2], $jparam[0]);
    }
...
// End of finding the begining day Of months
    ...
    static function jdate($type, $maket = 'now', $time) {
//        if (!$time)
//            $time = time();
        //global $icmsConfig;
        ...
        $result = '';
        if (is_string($maket)) {
            $year = date('Y', strtotime($maket, $time));
            $month = date('m', strtotime($maket, $time));
            $day = date('d', strtotime($maket, $time));
            list( $jyear, $jmonth, $jday ) = self::gregorian_to_jalali($year, $month, $day);
            $need = self::jmaketime(date('h', strtotime($maket, $time)), date('i', strtotime($maket, $time)), date('s', strtotime($maket, $time)), $jmonth, $jday, $jyear);
        } else {
    ...
    public static function fromUNIXformat($UnixDateString) {
        $f1 = date("Y-m-d H:i:s", strtotime($UnixDateString));
        $ar = explode(' ', $f1);
        return implode(' ', [Timely::AllCalendartToJalali($ar[0]), $ar[1]]);
    }
