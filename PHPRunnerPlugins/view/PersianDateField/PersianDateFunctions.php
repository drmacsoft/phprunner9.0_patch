...
class PersianDateFunctions{
	...
	function format_datetime_custom($time, $format) {
		...
		$i = 0;
		$subst = array();
		$weekday = $this->getdayofweek($time);
	 ...
	 * @intellisense
	 */
	function format_persiandate_custom($time, $format) {
	...
	function str_format_datetime($time) {
		global $locale_info;
		return $this->format_datetime_custom($time, $locale_info["LOCALE_SSHORTDATE"] . " " . $locale_info["LOCALE_STIMEFORMAT"]);
	}
	...
	function str_format_time($time) {
		global $locale_info;
		return $this->format_datetime_custom($time, $locale_info["LOCALE_STIMEFORMAT"]);
	}
	...
	function format_shortdate($time) {
		global $locale_info;
		return $this->format_datetime_custom($time, $locale_info["LOCALE_SSHORTDATE"]);
	}

	/**
	 * @intellisense
	 */
	function format_persiandate($time) {
		global $locale_info;
		return $this->format_persiandate_custom($time, $locale_info["LOCALE_SSHORTDATE"]);
	}
	...
	function format_longdate($time) {
		global $locale_info;
		return $this->format_datetime_custom($time, $locale_info["LOCALE_SLONGDATE"]);
	}
	...
	function localdate2db($strdate) {
		global $locale_info;
		return $this->simpledate2db($strdate, $locale_info["LOCALE_IDATE"]);
	}
	...
	//	determine am/pm
		...
		$numbers = $this->parsenumbers($strdatetime);
		...
		if ($time[0] >= 2004) {
			...
			for ($i = 2003; $i >= $time[0]; $i--)
				if (isleapyear($i))
					...
					$daydif-=365;
	//	to given month
		$mdays = array(1 => 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31);
	...
	function addHours($dateArray, $hours) {
		$days = floor($hours / 24);
		if ($days) {
			$dateArray = $this->adddays($dateArray, $days);
			$hours = $hours - ($days * 24);
		}
		...
		if ($dateArray[3] + $hours < 24)
			...
			$dateArray = $this->adddays($dateArray, 1);
	...
	function addMinutes($dateArray, $minutes) {
		...
		if ($hours) {
			$dateArray = $this->addHours($dateArray, $hours);
		...
		if (($dateArray[4] + $minutes) < 60)
			...
			$dateArray = $this->addHours($dateArray, 1);
	...
	function getLastMonthDayNumber($year, $month) {
		...
		$mdays = array(31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31, 31);
